...
export class OhioCountyCodeList {
    ...
    keyToValueDisplayString(code: string): string {
        return `${code} - ${(this.items.get(code) as OhioCountyCode).description}`;
    }

    constructor() {
        this.items = new Map<string, OhioCountyCode>();
