...
export class StateAbbreviationList {
    ...
    keyToValueDisplayString(abbreviation: string): string {
        return `${abbreviation} - ${(this.items.get(abbreviation) as StateAbbreviation).name}`;
    }

    constructor() {
        this.items = new Map<string, StateAbbreviation>();
